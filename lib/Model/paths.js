...
Model.prototype._splitPath = function(subpath) {
  ...
  if (Array.isArray(subpath)) {
    return (segments && segments.concat(subpath)) || subpath;
  } else if (subpath) {
    return (segments && segments.concat(subpath.split('.'))) || subpath.split('.');
  } else {
...
Model.prototype.path = function(subpath) {
  ...
  if (Array.isArray(subpath)) {
    return (this._atSegments) ? this._atSegments.concat(subpath) : subpath;
  }
...
function createScoped(model, path) {
  ...
  if (!path) {
    scoped._at = path;
    scoped._atSegments = path;
  } else if (Array.isArray(path)) {
    scoped._at = path.join('.');
    scoped._atSegments = path;
  } else {
    scoped._at = path;
    scoped._atSegments = path.split('.');
  }

  return scoped;

  // scoped._at = path;
  // scoped._atSegments = pathSegments;
  // return scoped;
}
